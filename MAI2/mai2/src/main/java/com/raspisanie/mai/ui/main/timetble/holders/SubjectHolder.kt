...
class SubjectHolder(itemView: View) : DFBaseHolder(itemView) {
    ...
    override fun bind(data: Any?) {
        ...
        with(itemView) {
            ...
            vgOnlineLink.visibility = if (subject.link.isNotEmpty()) View.VISIBLE else View.GONE
            cvSubject.strokeColor = if (subject.link.isNotEmpty())
                selected else unselected
            vgOnlineLink.setOnClickListener {
                makeEvent(TimetablePresenter.OPEN_ONLINE, subject.link)
            }
        }
    }
}